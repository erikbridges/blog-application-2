...
export default async (req, res, next) => {
    ...
    try {
        switch (editWhich) {
            ...
            default: {
                res.send(new Errors.UnprocessableEntityError("Edit which is not specified."))
                return next()
            }
        }

    } catch (ex) {
        res.send(new Errors.InternalServerError("Something is wrong with our servers at the time." + ex.message))
        return next();
    }
